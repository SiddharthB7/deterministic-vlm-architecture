"""
Mistral Intent Extractor (Brain 1 - Router)
-------------------------------------------
...
class MistralIntentRouter:
    ...
    def classify(self, user_text: str) -> Dict[str, Any]:
        ...
        if not user_text:
            return {"mode": "chat", "targets": [], "confidence": 0.0}
...
JSON:"""
    ...
    def _normalize(self, data: Dict[str, Any], user_text: str, hint: bool) -> Dict[str, Any]:
        ...
        return {"mode": mode, "targets": clean_targets, "confidence": confidence}

    def _merge_with_heuristic(self, model_result: Dict[str, Any], heuristic: Dict[str, Any]) -> Dict[str, Any]:
        if heuristic["confidence"] > model_result.get("confidence", 0.0):
            return heuristic

        if heuristic["mode"] != "chat" and model_result["mode"] == "chat":
            return heuristic

        if not model_result.get("targets") and heuristic.get("targets"):
            model_result["targets"] = heuristic["targets"]

        return model_result

    def _heuristic_classify(self, user_text: str, hint: bool) -> Dict[str, Any]:
        text = user_text.lower().strip()
        cleaned = re.sub(r"\s+", " ", text)

        if self._is_assistant_identity_question(cleaned):
            return {"mode": "chat", "targets": [], "confidence": 0.995}

        if cleaned in CHAT_PATTERNS or cleaned.startswith(("hello ", "hi ", "hey ")):
            return {"mode": "chat", "targets": [], "confidence": 0.99}

        if any(pattern in cleaned for pattern in SCENE_PATTERNS):
            return {"mode": "vision_scene", "targets": [], "confidence": 0.95}

        if self._looks_like_attribute_question(cleaned):
            target = self._extract_target(cleaned, default="object")
            return {"mode": "vision_attribute", "targets": [target] if target else [], "confidence": 0.9}

        if any(cleaned.startswith(prefix) for prefix in FIND_PREFIXES):
            target = self._extract_target(cleaned)
            return {"mode": "vision_find", "targets": [target] if target else [], "confidence": 0.88}

        if hint and self._contains_visual_question(cleaned):
            target = self._extract_target(cleaned)
            mode = "vision_attribute" if target else "vision_scene"
            targets = [target] if target else []
            return {"mode": mode, "targets": targets, "confidence": 0.8}

        return {"mode": "chat", "targets": [], "confidence": 0.4}
